...
def _team_name(team: Dict[str, Any]) -> str:
    """Nombre limpio del equipo."""
    return _esc(team.get("name", "?"))
...
def _league_line(fixture: Dict[str, Any]) -> str:
    ...
    # Round suele venir como "Regular Season - 38" → mostramos solo "J38"
    round_short = ""
    if round_:
...
def msg_match_finished(fixture: Dict[str, Any]) -> str:
    ...
    status_extra = (fixture.get("fixture", {}).get("status", {}) or {}).get("long", "")
...
def msg_goal(event: Dict[str, Any], fixture: Dict[str, Any]) -> str:
    ...
    if isinstance(assist, dict):
        ...
        assist_name = assist
    assist_line = f"\n🅰️ Asistencia: {_esc(assist_name)}" if assist_name else ""
...
def msg_substitution(event: Dict[str, Any], fixture: Dict[str, Any]) -> str:
    ...
    inn = event.get("player", "")
    inn_name = _esc(inn)
    out = event.get("assist", "")
    out_name = _esc(out)
    minute = _minute_str(event)
