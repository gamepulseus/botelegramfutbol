...
class FootballBot:
    ...
    def _process_fixture(self, fixture: Dict[str, Any]) -> None:
        ...
        # Inicio de partido (NS → 1H o LIVE)
        if (
            not fs.started_announced
            and prev_status in STATUS_NOT_STARTED.union({""})
        ...
        if (
            ...
            and prev_status in STATUS_FIRST_HALF
        ...
        if (
            ...
            and prev_status in STATUS_HALFTIME
            ...
            self.state.update(
                ...
                lambda s: (
                    setattr(s, "second_half_announced", True),
                    setattr(s, "halftime_announced", False),  # reset por si hubo extra time
                ),
            )
    ...
    def _process_events(
        ...
        Si is_new_fixture=True (primera vez que vemos este partido y ya estaba
        en curso), marca TODOS los eventos actuales como ya publicados sin
        enviarlos, para no inundar el canal con notificaciones retroactivas.
        """
        if not events:
            return
        ...
        for ev in events:
            ...
            # Si es primer contacto, NO publicar: solo marcar como visto.
            if is_new_fixture:
                continue

            msg = self._format_event(ev, fixture)
            if msg is None:
                # Tipo de evento que no notificamos: lo marcamos como publicado
                # para no reintentar.
                continue
            self._post(msg)
    ...
    def _format_event(
        ...
        # Gol (incluye penaltis convertidos, autogoles; excluye penaltis fallados
        # que también son type=Goal pero detail=Missed Penalty)
        if ev_type == "Goal":
        ...
        # Penalti señalado (algunas ligas reportan type=Goal detail=Penalty
        # cuando ya se cobra; aquí usamos el VAR con detail=Penalty confirmed)
